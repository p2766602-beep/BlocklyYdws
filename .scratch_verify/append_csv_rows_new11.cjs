...
const files = [
  { file: '114EChanghua.js', batch: '114-Changhua-E' },
  { file: '114ETaipei.js', batch: '114-Taipei-E' },
  { file: '114ETaitung.js', batch: '114-Taitung-E' },
  { file: '114JChaiyiC.js', batch: '114-ChaiyiC-J' },
  { file: '114JChanghua.js', batch: '114-Changhua-J' },
  { file: '114JHsinchu.js', batch: '114-Hsinchu-J' },
  { file: '114JHualien.js', batch: '114-Hualien-J' },
  { file: '114JNewTaipei.js', batch: '114-NewTaipei-J' },
  { file: '114JTainan.js', batch: '114-Tainan-J' },
  { file: '114JTaipei.js', batch: '114-Taipei-J' },
  { file: '114JTaitung.js', batch: '114-Taitung-J' },
];

const rows = [];
for (const { file, batch } of files) {
  const course = loadCourse(fs.readFileSync(`${CANONICAL_DIR}/${file}`, 'utf8'));
  const level = course.code.startsWith('114J') ? '國中' : '國小';
  course.tasks.forEach((t, idx) => {
    ...
    row[40] = '良好';
    row[41] = '低';
    row[42] = `Claude手動草擬並用headless Blockly harness驗證starterXml，${batch}批次，2026-08批次改用txtFile資料源。`;
    row[43] = t.title;
    row[44] = '是';
