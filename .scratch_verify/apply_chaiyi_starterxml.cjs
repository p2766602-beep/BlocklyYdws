// 把verify.mjs驗證過100% pass的starterXml，依序套進114EChaiyi.js/114JChaiyi.js
// 6份檔案（YDWS-CodingBank canonical + BlocklyYdws/blockly-lab各自的複製）。
// 每份檔案裡"starterXml": ""依task順序出現9次，跟tasks_chaiyi_e/j.json的task順序
// (task 1~9)一一對應，依序替換即可。
const fs = require('fs');
const path = require('path');

const eTasks = JSON.parse(fs.readFileSync(path.join(__dirname, 'tasks_chaiyi_e.json'), 'utf8'));
const jTasks = JSON.parse(fs.readFileSync(path.join(__dirname, 'tasks_chaiyi_j.json'), 'utf8'));

const targets = [
  { file: 'D:/YOSEP/YDWS-CodingBank/courses/114EChaiyi.js', tasks: eTasks },
  { file: 'D:/YOSEP/BlocklyYdws/src/courses/114EChaiyi.js', tasks: eTasks },
  { file: 'D:/YOSEP/blockly-lab/src/courses/114EChaiyi.js', tasks: eTasks },
  { file: 'D:/YOSEP/YDWS-CodingBank/courses/114JChaiyi.js', tasks: jTasks },
  { file: 'D:/YOSEP/BlocklyYdws/src/courses/114JChaiyi.js', tasks: jTasks },
  { file: 'D:/YOSEP/blockly-lab/src/courses/114JChaiyi.js', tasks: jTasks },
];
