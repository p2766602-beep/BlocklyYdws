// 修正114TCPJ16（雲林縣競賽模式）：從已修正的114JYunlin重新產生競賽模式版本，
// 比照build_tcp14_16.cjs既有規則，但只處理這一個課程代碼。
const fs = require('fs');
...
// 2. BlocklyYdws：先寫入完整版（含expectedOutput），供build-answer-key擷取，稍後strip
