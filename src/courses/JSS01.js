  ...
  "tasks": [
    {
      ...
      "difficulty": "L1",
      "blocklyFit": "高",
      "description": "輸入一個字串，請輸出這個字串的長度。本題用來示範把文字視為資料並取得長度。",
    ...
    },
    {
      ...
      "difficulty": "L1",
      "blocklyFit": "高",
      "description": "輸入一個字串，請輸出第一個字元與最後一個字元，兩個結果各佔一行。本題用來示範字串索引與位置概念。",
      "inputDescription": "第一行：一個不含空白的字串 S。",
    ...
    },
    {
      ...
      "statement": {
        "description": "七段顯示器是一種常見的數字顯示裝置，由 7 條 LED 燈條組成，可用來顯示數字 0~9。\n\n每個數字所需點亮的 LED 燈條數量如下：\n\n0→6條、1→2條、2→5條、3→5條、4→4條、5→5條、6→6條、7→3條、8→7條、9→6條。\n\n現在給定 N 條可用的 LED 燈條，請你使用這些燈條組成一個「數字（可為雙位數）」，並且必須符合以下規則：\n\n1. 數字（0~9）最多只能各使用一次（兩位數時十位與個位不能相同）。\n\n2. 組成的數字不可有前導零（除非答案本身就是 0）。\n\n3. 組成的數字最多只能有 2 位（0~99）。\n\n請設計一個程式，找出在符合上述條件下「剛好用完所有 N 條 LED 燈條」時，可以組成的最大值。\n\n若無法用 1 位數或 2 位數剛好用完所有 N 條 LED 燈條，請輸出 -1。",
        "input": "一行輸入一個整數 N（2 ≤ N ≤ 30），代表可用的 LED 燈條數。",
        "output": "一行輸出可組成的最大值（最大值 < 100），若不存在則輸出 -1。"
      },
      "examples": [
        {
          "input": "3",
    ...
    },
    {
      ...
      "difficulty": "L3",
      "blocklyFit": "中",
      "description": "考古學家在遺跡中發現了兩塊圓形的能量石板，上面刻著古老的數字代碼。根據解讀，這兩塊石板具有「相位旋轉」的特性。若想啟動遺跡的核心能源必須將這兩股最強的能量數值進行「大數加法」融合，所得的總和就是啟動核心的密碼。 請你編寫程式，根據考古學家輸入的兩塊石板數字，計算出融合後的最終能量數值 。\n\nNote: Scratch與 Blockly 積木運算大於16位的數字時已無法精確運算，因此輸入及輸出的數字字串最後方會強制加上一個英文字母 a (例如 12345a)， 請將其視為字串處理，讀取時請記得忽略最後一個字元，輸出時請用字串結合的方式在最後面結合一個英文字母a(詳見範例)。",
